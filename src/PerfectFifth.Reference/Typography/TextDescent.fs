module P5Reference.Typography.TextDescent
...
let draw p5 =
    ...
    let desc = textDescent p5 * scalar // Calc ascent
    line p5 0 (baseHeight + desc) width (baseHeight + desc)
    text p5 "dp" 0 baseHeight // Draw text on baseline

    setTextSize p5 64 // Increase text size
    let desc' = textDescent p5 * scalar // Recalc ascent
    line p5 40 (baseHeight + desc') width (baseHeight + desc')
